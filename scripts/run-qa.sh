#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="/Users/rishi/Desktop/WorkSpace/NiftyOptionPriceCalculatore"
API_DIR="$ROOT_DIR/nifty-auth-api"
WEB_DIR="$ROOT_DIR/NiftyOptionCalculator-Website-React"

# Start API (qa)
(
  cd "$API_DIR"
  set -a
  source .env.qa
  set +a
  nohup venv/bin/uvicorn app.main:app --host 127.0.0.1 --port 8000 > /tmp/optix-api.log 2>&1 &
  echo "API (qa) started: http://127.0.0.1:8000"
)

# Start Website (qa)
(
  cd "$WEB_DIR"
  nohup npm run dev -- --host 127.0.0.1 --port 5173 --mode qa > /tmp/nifty-web.log 2>&1 &
  echo "Website (qa) started: http://127.0.0.1:5173"
)
